...
const extractImports = (code: string, ignoredModules: string[] = []): [string[], string[]] => [(code.match(importModuleRegexp) || [])
  .map((x) => x.replace(importModuleRegexp, '$1'))
  .filter((x) => !ignoredModules.includes(x)), (code.match(importCssRegexp) || [])
...
const interopRequireDefault = (obj: any) => {
  return obj && obj[Symbol.toStringTag] === 'Module' ? {...obj, __esModule: true} : obj
}
...
const defaultImportsRevolvor = (moduleImports: string[], cssImports: string[]): Promise<[Record<string, any>, CSSStyleSheet[]]> => {
  return Promise.all([Promise.all(moduleImports.map((module) => import(/* @vite-ignore */ normalizeModule(module)))).then((result) => Object.fromEntries(moduleImports.map((module, idx) => [module, interopRequireDefault(result[idx]),]))), Promise.all(cssImports.map((module) => importCss(normalizeModule(module)))).then((result) => result.map((x) => x?.default).filter(Boolean)),]).then((result) => result)
}

export type UseAsyncRunnerProps = Omit<UseRunnerProps, 'code'> & {
  files: Record<string, string>
  resolveImports?: (moduleImports: string[], cssImports: string[]) => Promise<[Record<string, any>, CSSStyleSheet[]]>
}
...
export const useAsyncRunner = ({
  ...
  const [state, setState] = useState<UseAsyncRunnerReturn>({
    isLoading: false, element: null, styleSheets: [], error: null,
  })
  ...
  useEffect(() => {
    ...
    const extractedImports = extractImports(trimmedCode, Object.keys(scopeRef.current?.import || {}))
    ...
    resolveImports(...extractedImports)
      .then(([importsMap, styleSheets]) => {
        ...
        Object.keys(files).forEach((name) => {
          if (name.endsWith('.css')) cssFiles[name] = normalizeCss(files[name]); else if (name !== 'App.tsx') jsFiles[name] = normalizeJs(files[name])
        })
          ...
          code, scope: withFiles({
            ...scopeRef.current, import: {
              react: React, ...Object.keys(cssFiles).reduce((acc, name) => {
                acc[`./${name}`] = undefined
                return acc
              }, {} as Record<string, undefined>), ...scopeRef.current?.import, ...importsMap,
            },
          }, jsFiles), onRendered: (error) => {
            if (controller.signal.aborted) return
            if (error) {
              setState({
                isLoading: false,
  ...
  }, [files, scope, disableCache, resolveImports])
